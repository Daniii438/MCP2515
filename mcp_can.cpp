...
{
    INT8U i;
  if(newmode == MCP_SLEEP){
    mcp2515_setRegister(MCP_CANCTRL, B10000000); //Config mode
    mcp2515_setRegister(MCP_CNF3,    B01000000); //Wakeup filter
    mcp2515_setRegister(MCP_CANCTRL, B00100000); //Sleep mode
    mcp2515_setRegister(MCP_CANINTE, B01000000); //Attach Interrupt for sleepmode
    mcp2515_setRegister(MCP_CANINTF, 0); //Clear Interrupt register
  }
  if(newmode == MCP_NORMAL) mcp2515_setRegister(MCP_CANINTF, 0);
...
** Descriptions:            Public function
*********************************************************************************************************/
void MCP_CAN::digitalPinWrite(INT8U MCPPin, INT8U OutputState){
  INT8U digitalPinRegister = mcp2515_readRegister(MCP_BFPCTRL); //digitalPins

  if (MCPPin == 10) {
    bitWrite(digitalPinRegister, 5, OutputState);
    mcp2515_setRegister(MCP_BFPCTRL, digitalPinRegister); //Set digital pins

  }
  if (MCPPin == 11) {
    bitWrite(digitalPinRegister, 4, OutputState);
    mcp2515_setRegister(MCP_BFPCTRL, digitalPinRegister); //Set digital pins
  }
...
** Descriptions:            Public function
*********************************************************************************************************/
void MCP_CAN::InitPinIOModes(){
  mcp2515_setRegister(MCP_BFPCTRL, B00001100); //Set digital pins to output pins
  mcp2515_setRegister(MCP_TXRTSCTRL, B00000000); //Set analog pins to input pins
}
...
** Descriptions:            Public function
*********************************************************************************************************/
bool MCP_CAN::digitalPinRead(byte MCPPin){

  byte digitalPinRegister = mcp2515_readRegister(MCP_BFPCTRL);
  byte analogPinRegister = mcp2515_readRegister(MCP_TXRTSCTRL);

  if (MCPPin == 10) return bitRead(digitalPinRegister, 5);      //RX1BF
  else if (MCPPin == 11) return bitRead(digitalPinRegister, 4); //RX0BF
  else if (MCPPin == 4) return bitRead(analogPinRegister, 3);   //TX0RTS
  else if (MCPPin == 5) return bitRead(analogPinRegister, 4);   //TX1RTS
  else if (MCPPin == 6) return bitRead(analogPinRegister, 5);   //TX2RTS
  return -1;
}
